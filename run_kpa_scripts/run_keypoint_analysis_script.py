...
input_csv = '../dataset/echr_arguments.csv'

final_output_file = '../dataset/results_final.csv'
...
def run_debater():
    ...
    file = input_csv
    comment_ids_column = 'id'
    comment_text_column = 'argument'
    domain = DOMAIN
    host = HOST
    # ======================= update params =======================
    ...
    # how to run
    domain_params = {}
    run_params = {'sentence_to_multiple_kps': False, 'mapping_threshold': 0.9, }
...
if __name__ == '__main__':
    ...
    dataframe = pd.read_csv("../dataset/echr_arguments_kpa_results.csv")
    filter_none_data = dataframe[dataframe['kp'] != 'none']
    sorted_df = filter_none_data.sort_values(by=['comment_id', 'sentence_id'])
    ...
    res.to_csv(final_output_file, index=False)